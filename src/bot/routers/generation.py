...
async def start_generation(m: Message, state: FSMContext, show_intro: bool = True) -> None:
    ...
    # ✅ ДОБАВЛЕНО: Удалить старый файл результата при начале новой генерации
    try:
        data = await state.get_data()
        old_file_path = data.get("file_path")
        if old_file_path and os.path.exists(old_file_path):
            os.unlink(old_file_path)
            log.info(f"🗑️ Deleted old result on new generation: {old_file_path}")
...
async def _finalize_to_prompt(m: Message, state: FSMContext) -> None:
    _cancel_debounce(m.chat.id)  # ✅ отменяем таймер
...
def _schedule_album_finalize(m: Message, state: FSMContext, delay: float = 2.0):
    async def _debounce():
        try:
            ...
            # ✅ ИСПРАВЛЕНО: правильная обработка отмены
            return
        except Exception as e:
            log.exception(f"Debounce error: {e}")
        finally:
            # ✅ ВАЖНО: удаляем из словаря только после завершения
            _DEBOUNCE_TASKS.pop(m.chat.id, None)
...
async def send_generation_result(
    ...
    try:
        ...
        async with SessionLocal() as s:
            user = (await s.execute(select(User).where(User.chat_id == chat_id))).scalar_one_or_none()
            current_model = user.model_preference if user else "standard"

...
router = Router()
log = logging.getLogger(__name__)  # ✅ ДОБАВЛЕНО
...
async def cmd_create(m: Message, state: FSMContext):
    ...
    # ✅ ДОБАВЛЕНО: Удалить старый файл результата
    try:
        data = await state.get_data()
        old_file_path = data.get("file_path")
        if old_file_path and os.path.exists(old_file_path):
            os.unlink(old_file_path)
            log.info(f"🗑️ Deleted old result on /create: {old_file_path}")
    except Exception as e:
        log.warning(f"Failed to delete old result: {e}")
...
#     caption = (
#         "🍌 <b>Добро пожаловать в Nano Banana 2 pro</b> — Ai фотошоп от Google в удобном телеграм-боте:\n\n"
...
#         "Чтобы воспользоваться ботом, нажмите «Меню» в левом нижнем углу и выберите модель standard или <b>pro</b>👇\n\n"
...
async def cmd_start(m: Message):
    ...
    caption = (
        ...
        # "Рекомендуем изучить инструкцию перед использованием!\n"
        # "📖 <a href=\"https://t.me/nano_banana_examples\">Инструкция и примеры</a>\n\n"
        "Чтобы воспользоваться ботом, нажмите «Меню» в левом нижнем углу или кнопку «Выбрать модель»👇\n\n"
        "Пользуясь ботом, Вы принимаете наше "
        "<a href=\"https://docs.google.com/document/d/139A-rEgNeA6CrcOaOsOergVVx4bUq8NFlTLx4eD4MfE/edit?usp=drivesdk\">пользовательское соглашение</a> "
        "и <a href=\"https://telegram.org/privacy-tpa\">политику конфиденциальности</a>."
    )
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[[InlineKeyboardButton(text="🍌 Выбрать модель", callback_data="select_model")]]
    )

    await safe_send_text(m.bot, m.chat.id, caption, reply_markup=keyboard,disable_web_page_preview=True)


@router.callback_query(F.data == "select_model")
async def cb_select_model(c: CallbackQuery, state: FSMContext):
    """Показ меню выбора модели"""
    await safe_answer(c)
    await state.clear()
    
    # ✅ ИСПРАВЛЕНО: используем c.from_user.id напрямую
    async with SessionLocal() as s:
        user = (await s.execute(select(User).where(User.chat_id == c.from_user.id))).scalar_one_or_none()
        if not user:
            await safe_send_text(c.bot, c.message.chat.id, "Нажмите /start для инициализации.")
            return
        ...
        await safe_send_text(c.bot, c.message.chat.id, text, reply_markup=keyboard)
    ...
    try:
        ...
        await safe_send_text(
            ...
            (
                ...
                # f"<b>Standard</b>: 1 генерация — 1 изображение.\n"
                # f"<b>Pro</b>: 5 генераций — 1 изображение.\n\n"
...
async def cmd_example(m: Message):
    caption = (
        "📌 <b>Примеры работ Nano Banana 2 </b>\n\n"
...
@router.callback_query(F.data.startswith("model_"))
async def cb_model_select(c: CallbackQuery, state: FSMContext):
    """✅ Обработка выбора модели с автоматическим запуском генерации"""
    await safe_answer(c)
    ...
    async with SessionLocal() as s:
        ...
        await safe_edit_text(
            c.message,
            f"✅ Модель изменена на <b>{model_name}</b> ({cost} {'генерация' if cost == 1 else 'генераций'})"
        )

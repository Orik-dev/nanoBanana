...
async def _send_with_delete(bot, chat_id: int, message_id: int, text: str, reply_markup):
    """Удаляет с>ое сообщение и отправляет новое - для исправления навигации"""
    try:
...
# Только ИСПРАВЛЕННАЯ функция waiting_email
...
async def waiting_email(m: Message, state: FSMContext):
    ...
    async with SessionLocal() as s:
        ...
        if email.lower() in {"не нужен", "ненужен", "skip"}:
            ...
            # ✅ Используем validate_email из payments.py
            from services.payments import validate_email
            
            validated = validate_email(email)
            
            if not validated:
                log.warning(f"⚠️ Invalid email: user={m.from_user.id}, email={email[:50]}")
                await safe_send_text(
                    ...
                    "• Только латиница\n\n"
                    "Введите снова или напишите «не нужен»"
                )
                return
    ...
    try:
        ...
        log.exception(f"Payment creation failed: {e}")
        await safe_send_text(
            m.bot, m.chat.id,
            "⚠️ Ошибка создания платежа.\n"
            "Попробуйте позже или напишите @guard_gpt"
        )

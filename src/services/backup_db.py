    ...
    try:
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                # ✅ ДОБАВЛЕНО: специальная обработка Error 1317
                if "error 1317" in result.stderr.lower() or "query execution was interrupted" in result.stderr.lower():
                    log.error("❌ Mysqldump was interrupted - possible server overload")
                    ...
                    if settings.ADMIN_ID:
                        try:
                            ...
                            if bot:
                                await bot.send_message(
                                    settings.ADMIN_ID,
                                    "❌ <b>Backup Interrupted</b>\n\n"
                                    "Mysqldump was killed (Error 1317)\n"
                                    "Possible reasons:\n"
                                    "• Server overload\n"
                                    "• Long-running queries\n"
                                    "• Insufficient resources\n\n"
                                    "Consider:\n"
                                    "• Scheduling backups during low traffic\n"
        ...
        log.error("❌ Backup timeout (>20 min)")

...
async def process_generation(
    ...
    # ... остальные except БЕЗ ИЗМЕНЕНИЙ ...
        ...
        try:
            # ✅ ИМПОРТИРУЕМ escape_html
            from services.telegram_safe import escape_html
            ...
            if "file type not supported" in error_str or "not supported" in error_str:
                ...
                # ✅ ИСПРАВЛЕНО: экранируем сообщение от API
                msg = str(e).split(":", 1)[-1].strip() if ":" in str(e) else str(e)
                msg_escaped = escape_html(msg[:200])  # ✅ Экранируем HTML
                await bot.send_message(
                    chat_id,
                    f"⚠️ <b>Ошибка запроса</b>\n\n"
                    f"{msg_escaped}\n\n"  # ✅ Используем экранированный текст

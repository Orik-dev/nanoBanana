...
class KieClient:
    ...
    async def create_task(
        ...
        *,
        ...
        # if len(prompt) < 3:
        #     raise KieError("Prompt too short (minimum 3 characters)")
        prompt = (prompt or "").strip()
        if not prompt:
            raise ValueError("prompt is empty")
        ...
        if len(prompt) < MIN_PROMPT_LENGTH:
            log.warning(_j(
                "kie.create.prompt_too_short",
                cid=cid,
                length=len(prompt),
                min_required=MIN_PROMPT_LENGTH
            ))
            raise ValueError(f"Prompt too short (min {MIN_PROMPT_LENGTH} chars)")    
